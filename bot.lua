...
local bot_api_key = "297139598:AAFNfCzbHPIrzU5hasoKOwvdgUSlUwOaqhY" --------------Youre bot token
...
function msg_processor(msg)
  ...
  elseif msg.photo then
	...
	elseif msg.text:match("^/help") or msg.text:match("^help") then
    local matches = { string.match(msg.text, "^start$") }
      local text = "*Show command* _:_"
      local key = {}
     key.inline_keyboard = {
	 {{text = '\nTo photo : send sticker', callback_data = 'tophoto'}},
	 {{text = '\nTo sticker : send photo', callback_data = 'tosticker'}}
	 }
	 sendMessage(msg.chat.id, text, true, false, true, key)
elseif msg.text:match("^/start") or msg.text:match("^start") then
    local matches = { string.match(msg.text, "^start$") }
      local text = "*Welcome Youre Friend*"
      local key = {}
     key.inline_keyboard = {{{text = '\nShow command : help', callback_data = 'start'}}}
	 sendMessage(msg.chat.id, text, true, false, true, key)
	 ------------------------------------------------------------------------
      local key = {}
      key.keyboard = {{{text = 'help'},{text = 'start'}}}
             sendMessage(msg.chat.id, text, true, false, true, key)
return end
end
bot_run() 
